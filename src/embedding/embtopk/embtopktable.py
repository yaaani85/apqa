...
#  EmbTopKEDBTable("E", "<http://www.lehigh.edu/~zhp2/2004/0401/univ-bench.owl#memberOf>", 10, edb, model) 
class EmbTopKEDBTable(PyTable):
    def __init__(self, predname, relname, top_k, edb_layer, model : KgeModel, entities_dict : dict = None,
        ...
        self.model = model
        # kbc, epoch, loss = kbc_model_load("/home/yannick/phd/coding/cqd/models/FB15k-237-model-rank-1000-epoch-100-*.pt")
        # self.kbc_model = kbc.model
        # self.model.eval()
        ...
        for id, rel in enumerate(self.model.dataset.relation_ids()):
            if f"<{rel}>".strip() == relname.strip():
                self.rel_id = id
        if self.rel_id is None:
            raise Exception(f"Relation not found{self.rel_name} ")
        if entities_dict is not None:
            ...
            self.entities_dict = {}
            for idx, e in enumerate(self.model.dataset.entity_ids()):
               
                # self.entities_dict[e] = idx
                self.entities_dict[f"<{e}>"] = idx
    ...
    def get_iterator(self, t: tuple) -> PyIterator:
        ...
        if t[0].is_variable():
            ...
            txt_top_k_s = self.model.dataset.entity_ids(top_k_s)
            for idx, e in enumerate(txt_top_k_s):
                answers.append(( f"<{e}>", value, top_k_scores[idx].item()))
        elif t[1].is_variable():
            # Retrieve ID of the subject
            value = t[0].get_value()
            value_id = self.entities_dict[value]
            ...
            # print("Scoresshape", scores)
            # scores = F.sigmoid(scores)
            # m = nn.Sigmoid()
            # scores = m(scores)
            # print("Scoresafter", scores)
            ...
            if self.score_threshold is not None:
                ...
                top_k_scores = top_k_scores[retained_entities]
            txt_top_k_o = self.model.dataset.entity_ids(top_k_o)
            for idx, e in enumerate(txt_top_k_o):
                answers.append((value, f"<{e}>", top_k_scores[idx].item()))
        else:
